...
export const fetchData = async () => {
  ...
  const contract = new ethers.Contract(addresses['MintableERC20'], mintableErc20.abi, provider); // as MintableERC20;

  const web3Modal = new Web3Modal();
  const connection = await web3Modal.connect();
  const userProvider = new ethers.providers.Web3Provider(connection); //window.ethereum);
